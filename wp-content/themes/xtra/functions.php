...
if ( ! class_exists( 'Codevz_Theme' ) ) {
	...
	class Codevz_Theme {
		...
		public static function get_the_excerpt( $excerpt ) {
			...
			return $excerpt ? $excerpt . wp_kses_post( self::option( 'post_excerpt_dots', ' ... ' ) ) . $button : '';
		...
		public static function get_row_element( $i, $m = [] ) {
			...
			if ( $elm === 'logo' || $elm === 'logo_2' ) {
				...
				if ( $logo ) {
					...
					$sizes = self::$plugin ? Codevz_Plus::getimagesize( $logo ) : '';
					...
					if ( $sizes ) {
						...
						$lw = empty( $i[ 'logo_width' ] ) ? 'auto' : $i[ 'logo_width' ];
						$lh = 'auto';

					}

					$escaped_size_on_sticky = empty( $i[ 'logo_width_sticky' ] ) ? '' : ' data-cz-style=".onSticky .' . $element_id . $m['depth'] . ' .logo_is_img img{width:' . esc_attr( $i[ 'logo_width_sticky' ] ) . ' !important}"';

					$logo_html = '<div class="logo_is_img ' . esc_attr( $elm ) . '"><a href="' . self::$home_url . '" title="' . esc_html( get_bloginfo( 'description' ) ) . '"><img src="' . esc_url( $logo ) . '" alt="' . esc_attr( get_bloginfo( 'name' ) ) . '" width="' . esc_attr( $lw ) . '" height="' . esc_attr( $lh ) . '" style="width: ' . esc_attr( isset( $lp ) ? $lw . $lp : $lw ) . '"' . $escaped_size_on_sticky . '></a>';
				...
				echo '<a class="shop_icon noborder" href="' . esc_url( $cart_url ) . '" aria-label="' . esc_html( self::option( 'woo_cart', 'Cart' ) ) . '"><i class="' . esc_attr( $icon ) . '" style="' . esc_attr( $icon_style ) . '"></i></a>';
				...
				if ( $template === 'x' && $custom_template && $custom_template !== 'none' ) {
					...
					while ( have_posts() ) {
						...
						if ( $chess ) {
							...
							if ( self::contains( $template, [ '1', '2', '3', '4', '5', '6', '7' ] ) ) {
