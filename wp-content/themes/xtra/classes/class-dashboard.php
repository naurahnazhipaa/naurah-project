	...
	class Xtra_Dashboard {
		...
		public function enqueue( $hook ) {
			...
			wp_localize_script( 'xtra', 'xtraWizard', [
				...
				'error_500' 		=> esc_html__( 'PHP error 500, Internal server error.', 'xtra' ),
				'ajax_error' 		=> esc_html__( 'An error has occured, please check your internet connection and try again.', 'xtra' ),
		...
		private function render_before( $active = null ) {
			...
			foreach( $this->menus as $slug => $title ) {
				...
				echo '<a href="' . esc_url( admin_url( $link ) ) . '" class="' . esc_attr( $active === $slug ? 'xtra-current' : '' ) . '"><img src="' . Codevz_Theme::$url . 'assets/img/' . esc_attr( $img ) . '.png" /><span>' . esc_html( $title ) . '</span></a>';
		...
		private function icon_box( $icon, $title, $link, $class = '' ) {
			...
			echo '<a href="' . esc_url( $link ) . '" class="xtra-dashboard-icon-box' . esc_attr( $class ) . '"><i class="dashicons dashicons-' . esc_attr( $icon ) . '"></i><div>' . wp_kses_post( $title ) . '</div></a>';
			...
			if ( $action ) {
				...
				if ( $purchase_code ) {
						...
						if ( isset( $_POST[ 'register' ] ) ) {
							...
							if ( empty( $_POST[ 'register' ] ) ) {
								...
								if ( is_bool( $register ) ) {
									...
									$this->message( 'success', esc_html__( 'Congratulation', 'xtra' ) . ', ' . esc_html__( 'Your theme activated successfully.', 'xtra' ) );
					...
					if ( ! isset( $hide_form ) ) {
						...
						$this->icon_box( 'editor-help', esc_html__( 'How to find purchase code?', 'xtra' ), '#', 'info' );

						$this->icon_box( 'cart', esc_html__( 'Buy new license', 'xtra' ), '#', 'success' );
		...
		public function importer() {
			...
			echo '<div class="xtra-filters">';
				...
				echo '<a href="#" data-filter="js_composer">' . esc_html__( 'WPBakery', 'xtra' ) . '</a>';
			...
			foreach( $this->demos as $demo => $args ) {
				...
				$args[ 'demo' ] = $demo;
				$args[ 'image' ] = Codevz_Theme::$api . 'demos/' . esc_attr( $demo ) . '.jpg';
				$args[ 'preview' ] = str_replace( 'api', esc_attr( $demo ), Codevz_Theme::$api );
				...
				echo '<div class="xtra-demo">';
					...
					$keywords .= empty( $args[ 'elementor' ] ) ? '' : ' elementor';
					...
					// Demo title.
					echo '<div class="xtra-demo-title">' . ucwords( str_replace( '-', ' ', esc_html( $args[ 'demo' ] ) ) ) . '</div>';
				...
				echo '<div class="xtra-wizard-main">';
					...
					echo '<div class="xtra-wizard-content">';
						...
						echo '<div data-step="1" class="xtra-current">';
							...
							echo '<a href="#" class="xtra-live-preview xtra-button-secondary" target="_blank">' . esc_html__( 'Live Preview', 'xtra' ) . '</a>';
						...
						echo '<div data-step="2">';
							...
							echo '<label class="xtra-checkbox xtra-rtl" data-tooltip="' . esc_html__( 'By checking this field, wizard will import Arabic version of current demo that you have selected.', 'xtra' ) . '">' . esc_html__( 'RTL version?', 'xtra' ) . '<input type="checkbox" name="rtl" ' . ( is_rtl() ? 'checked' : '' ) . ' /><span class="checkmark"></span></label>';
		...
		public function uninstall() {
			...
			echo '<p class="xtra-uninstall-p">In this list you can see demos imported on your site previously, You can uninstall any demo data.</p>';
			...
			foreach ( $this->demos as $demo => $args ) {
				...
				if ( get_option( 'xtra_uninstall_' . $demo ) ) {
					...
					echo '<div class="xtra-demo">';
						...
						echo '<img data-src="' . esc_url( Codevz_Theme::$api . 'demos/' . esc_attr( $demo ) . '.jpg' ) . '" />';

						echo '<div class="xtra-demo-title">' . ucwords( str_replace( '-', ' ', esc_html( $demo ) ) ) . '</div>';
		...
		protected function install_plugin( $plugin = '' ) {
			...
			if ( ! file_exists( $this->plugin_file( $slug, true ) ) ) {
				...
				return [
					...
					'status' 	=> '202',
					'message' 	=> sprintf( esc_html__( 'Install failed for "%s" plugin, Please increase your server PHP max_execution_time e.g. 300', 'xtra' ), $slug )
